...
if [[ "$PRODUCTION" == "1" ]] || [[ "$IN_PRODUCTION" == "1" ]]; then
    ...
    echo log_level = warning >> /etc/php/php-fpm.conf
...
if [ ! -z "$PHP_MEMORY_LIMIT" ]; then
    sed -i "s/memory_limit = 128M/memory_limit = ${PHP_MEMORY_LIMIT}M/g" /etc/php/conf.d/php.ini
fi
...
# sed -i "s/pm.max_children = 5/pm.max_children = 70/g" /etc/php/php-fpm.d/www.conf
# sed -i "s/pm.start_servers = 3/pm.start_servers = 20/g" /etc/php/php-fpm.d/www.conf
# sed -i "s/pm.min_spare_servers = 2/pm.min_spare_servers = 20/g" /etc/php/php-fpm.d/www.conf
# sed -i "s/pm.max_spare_servers = 4/pm.max_spare_servers = 35/g" /etc/php/php-fpm.d/www.conf
